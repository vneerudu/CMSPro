...
import com.cmspro.gateway.config.KafkaProperties;
...
class CmsProGatewayServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        CmsProGatewayServiceKafkaResource kafkaResource = new CmsProGatewayServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cms-pro-gateway-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cms-pro-gateway-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
