...
import com.cmspro.microservice.config.KafkaProperties;
...
class CmsProMicroServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        CmsProMicroServiceKafkaResource kafkaResource = new CmsProMicroServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cms-pro-micro-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cms-pro-micro-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
