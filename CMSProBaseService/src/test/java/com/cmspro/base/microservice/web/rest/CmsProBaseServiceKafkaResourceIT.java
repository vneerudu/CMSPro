...
import com.cmspro.base.microservice.config.KafkaProperties;
...
class CmsProBaseServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        CmsProBaseServiceKafkaResource kafkaResource = new CmsProBaseServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cms-pro-base-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cms-pro-base-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
